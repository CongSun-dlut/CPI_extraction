...
class InputExample(object):
    ...
    def __init__(self, guid, title, text_a, text_b=None, sdp=None, label=None):
...
class InputFeatures(object):
    ...
    def __init__(self, title_ids, title_mask, title_segment, input_ids, input_mask, segment_ids, \
                 sdp_ids, sdp_mask, sdp_segment, P_gauss1_list, P_gauss2_list, label_id):
        
        self.title_ids=title_ids
        self.title_mask=title_mask
        self.title_segment = title_segment
...
class CPIProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            label = line[4]
            examples.append(
                InputExample(guid=guid, title=title, text_a=text_a, text_b=None, sdp=sdp, label=label))
        return examples
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        title = tokenizer.tokenize(example.title)
        if len(title) > max_seq_length - 2:
            title = title[:(max_seq_length - 2)]
        ...
        sdp = tokenizer.tokenize(example.sdp)
        if len(sdp) > max_seq_length - 2:
            sdp = sdp[:(max_seq_length - 2)]
        ...
        sdp_ids = tokenizer.convert_tokens_to_ids(sdp)

        # The mask has 1 for real tokens and 0 for padding tokens. Only real
        # tokens are attended to.
        title_mask = [1] * len(title_ids)
        input_mask = [1] * len(input_ids)
        sdp_mask = [1] * len(sdp_ids)
        ...
        padding_sdp = [0] * (max_seq_length - len(sdp_ids))
        sdp_ids += padding_sdp
        
        title_mask += padding_title
        input_mask += padding
        sdp_mask += padding_sdp
        
        title_segment += padding_title
        segment_ids += padding
        sdp_segment += padding_sdp
        ...
        assert len(sdp_ids) == max_seq_length
        assert len(sdp_mask) == max_seq_length
        assert len(sdp_segment) == max_seq_length
        ...
        if ex_index < 5:
            ...
            logger.info("title: %s" % " ".join([str(x) for x in title]))
            logger.info("title_ids: %s" % " ".join([str(x) for x in title_ids]))
            logger.info("title_mask: %s" % " ".join([str(x) for x in title_mask]))
            logger.info("title_segment: %s" % " ".join([str(x) for x in title_segment]))
            ...
            logger.info("sdp: %s" % " ".join([str(x) for x in sdp]))
            logger.info("sdp_ids: %s" % " ".join([str(x) for x in sdp_ids]))
            logger.info("sdp_mask: %s" % " ".join([str(x) for x in sdp_mask]))
            logger.info("sdp_segment: %s" % " ".join([str(x) for x in sdp_segment]))
        ...
        features.append(
                InputFeatures(title_ids=title_ids,
...
def main():
    ...
    ## Other parameters
    parser.add_argument("--bert_saved", default="", type=str,
                        help="saved model")
    ...
    if args.do_train:
        ...
        all_title_ids = torch.tensor([f.title_ids for f in train_features], dtype=torch.long)
        all_title_mask = torch.tensor([f.title_mask for f in train_features], dtype=torch.long)
        all_title_segment = torch.tensor([f.title_segment for f in train_features], dtype=torch.long)
        
        all_input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in train_features], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.long)
        
        all_sdp_ids = torch.tensor([f.sdp_ids for f in train_features], dtype=torch.long)
        all_sdp_mask = torch.tensor([f.sdp_mask for f in train_features], dtype=torch.long)
        all_sdp_segment = torch.tensor([f.sdp_segment for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(all_title_ids, all_title_mask, all_title_segment, all_input_ids, all_input_mask, all_segment_ids, all_sdp_ids, all_sdp_mask, all_sdp_segment, all_P_gauss1_list, all_P_gauss2_list,all_label_ids)
        ...
        output_model_file = os.path.join(args.bert_saved, 'pytorch_model.bin')
        output_config_file = os.path.join(args.bert_saved, 'bert_config.json')
        config = BertConfig(output_config_file)
        model = BertForSequenceClassification(config, num_labels=num_labels)
        model.load_state_dict(torch.load(output_model_file))
    model.to(device)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_title_ids = torch.tensor([f.title_ids for f in eval_features], dtype=torch.long)
        all_title_mask = torch.tensor([f.title_mask for f in eval_features], dtype=torch.long)
        all_title_segment = torch.tensor([f.title_segment for f in eval_features], dtype=torch.long)
        
        all_input_ids = torch.tensor([f.input_ids for f in eval_features], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in eval_features], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in eval_features], dtype=torch.long)
        
        all_sdp_ids = torch.tensor([f.sdp_ids for f in eval_features], dtype=torch.long)
        all_sdp_mask = torch.tensor([f.sdp_mask for f in eval_features], dtype=torch.long)
        all_sdp_segment = torch.tensor([f.sdp_segment for f in eval_features], dtype=torch.long)
        ...
        eval_data = TensorDataset(all_title_ids, all_title_mask, all_title_segment, all_input_ids, all_input_mask, all_segment_ids, all_sdp_ids, all_sdp_mask, all_sdp_segment, all_P_gauss1_list, all_P_gauss2_list, all_label_ids)        
        ...
        for title_ids, title_mask, title_segment, input_ids, input_mask, segment_ids, sdp_ids, sdp_mask, sdp_segment, P_gauss1_list, P_gauss2_list, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
    ...
    if args.do_predict and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        all_title_ids = torch.tensor([f.title_ids for f in test_features], dtype=torch.long)
        all_title_mask = torch.tensor([f.title_mask for f in test_features], dtype=torch.long)
        all_title_segment = torch.tensor([f.title_segment for f in test_features], dtype=torch.long)
        
        all_input_ids = torch.tensor([f.input_ids for f in test_features], dtype=torch.long)
        all_input_mask = torch.tensor([f.input_mask for f in test_features], dtype=torch.long)
        all_segment_ids = torch.tensor([f.segment_ids for f in test_features], dtype=torch.long)
        
        all_sdp_ids = torch.tensor([f.sdp_ids for f in test_features], dtype=torch.long)
        all_sdp_mask = torch.tensor([f.sdp_mask for f in test_features], dtype=torch.long)
        all_sdp_segment = torch.tensor([f.sdp_segment for f in test_features], dtype=torch.long)
        ...
        predict_data = TensorDataset(all_title_ids, all_title_mask, all_title_segment, all_input_ids, all_input_mask, all_segment_ids, all_sdp_ids, all_sdp_mask, all_sdp_segment, all_P_gauss1_list, all_P_gauss2_list, all_label_ids)
        ...
        for title_ids, title_mask, title_segment, input_ids, input_mask, segment_ids, sdp_ids, sdp_mask, sdp_segment, P_gauss1_list, P_gauss2_list,label_ids in tqdm(predict_dataloader, desc="Predicting"):
            title_ids = title_ids.to(device)
            title_mask = title_mask.to(device)
            title_segment = title_segment.to(device)
            input_ids = input_ids.to(device)
